...
fine tuning, select 'Other...' which will open a new panel.
    ...
    __Note__: the first line of the matrix must contain
    the delays or wavelengths, depending on the choice of 
    `Data structure`.
    
    __Example__ of a CSV-type data file structure
...
* `Run` contains the controls for the optimizer.
  ...
  within this uncertainty box. 
    ...
    + `Restart` controls the initialization of the
      optimizer with the last results (should not be 
    ...
    - It is recommended to make a final run with `Restart`
      on to check that the optimized value is stable
...
* `Kinetics and Spectra` tab
    ...
    This tab provides zoomable plots of the spectra and the
    associated kinetics, identified by color code. 
    
    + `Plot uncertainty bands` plots the envelope of the set of
    spectra and kinetics enabled by the final uncertainty on the
    optimized model parameters. These correspond to the min and 
    max values generated from a random sample of parameters.
